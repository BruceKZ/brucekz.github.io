...
从任务角度看，`seq2seq` 最大的价值，是允许输入长度和输出长度都自由变化。
这和分类任务很不一样，后者通常只输出一个固定标签。
...
训练时常见做法是让 `decoder` 在每一步都看到真实历史目标词，也就是典型的 `teacher forcing` 思路。
它让训练更稳定，但也带来训练和推理阶段分布不完全一致的问题。
...
一个常见考点是：`attention` 并不是新的序列模型本体，而是给 `decoder` 增加了对源端各位置的动态访问能力。
它解决的是信息传递方式，而不是单独替代 `encoder` 或 `decoder`。
...
- **任务类型：** `seq2seq` 处理的是变长输入到变长输出的映射。
- **结构分工：** `encoder` 负责读入和压缩源序列，`decoder` 负责自回归生成目标序列。
- **训练目标：** 目标端仍然是在条件概率下做 `cross-entropy` 优化。

## 高频结论

- **核心瓶颈：** 早期 `seq2seq` 的主要问题是单向压缩形成的 `temporal bottleneck`。
- **Attention 的作用：** `attention` 让每一步解码都能重新访问全部 `encoder states`。
- **机制定位：** `attention` 是动态检索机制，不是对序列建模问题的完整替代。
